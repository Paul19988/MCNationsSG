...
public class WorldBorder {
    ...
    public void fadeBorder(Player p, int percentage, long time){
...
//		}
    ...
    public int getMaxPlayerHealth(Player p){
        return (int) ((Damageable) p).getMaxHealth();
    }

    public int getPlayerMissingHearts(Player p){
        return (int) ((Damageable) p).getMaxHealth();
    }

    public int getPlayerHealthPercentage(Player p){
        int health = getPlayerHealth(p);
        int maxhealth = getMaxPlayerHealth(p);
        return (Math.round((health * 100) / maxhealth));
    }

    public void disablePlayerTint(Player p){
        togglelist.add(p.getName());
    }

    public void enablePlayerTint(Player p){
    ...
    public boolean isTintEnabled(Player p) {
